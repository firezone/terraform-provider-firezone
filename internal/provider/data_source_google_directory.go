		...
		Attributes: map[string]schema.Attribute{
			...
			"name": schema.StringAttribute{
				...
				Description: "Directory name, as shown in the Firezone dashboard's identity provider settings. Set this " +
					"or id, not both. Lookup by name requires paginating every Google Workspace directory in the " +
					"account client-side (the API has no ?name= filter). Fails if more than one directory shares this name.",
			},
...
// findGoogleDirectoriesByName paginates every Google Workspace directory
// in the account looking for every exact name match, since the API has
// no ?name= query filter.
